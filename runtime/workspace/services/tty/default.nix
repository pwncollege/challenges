  ...
  ttydIndex =
    pkgs.runCommand "workspace-ttyd-index"
      ...
      ''
        ...
        {
          printf '<style>\n'
          printf '@font-face{font-family:"Hack";src:url(data:font/ttf;base64,'
          base64 -w0 ${pkgs.hack-font}/share/fonts/truetype/Hack-Regular.ttf
          printf ') format("truetype");font-weight:400;font-style:normal;font-display:block;}\n'
          printf '</style>\n'
          printf '<script>\n'
          printf 'window.__workspaceReadyTerminalFont=function(){if(!document.fonts)return Promise.resolve();var font=Promise.all([document.fonts.load('"'"'13px "Hack"'"'"'),document.fonts.ready]);var timeout=new Promise(function(resolve){setTimeout(resolve,1000)});return Promise.race([font,timeout]).catch(function(){})};\n'
          printf '</script>\n'
        } > font.html

        perl -0pi -e 'open my $fh, "<", "font.html" or die $!; local $/; my $font = <$fh>; s#</head>#$font</head>#' index.html
        perl -0pi -e 's#fontFamily:"Consolas,Liberation Mono,Menlo,Courier,monospace"#fontFamily:"Hack, monospace"#' index.html
