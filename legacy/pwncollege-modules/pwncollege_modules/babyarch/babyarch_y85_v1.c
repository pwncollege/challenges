...
typedef struct vmstate
{
    ...
    word_t memlimit; // defines the protected memory area
...
void sys_exec(vmstate_t *state, word_t addr);
...
int sys_read(vmstate_t *state, int fd, void *buf, size_t count)
{
    ...
    // pr_info("sys_read: fd %d file %#lx\n", fd, file);
    if (!file)
        return -1;
    const struct cred *old = override_creds(prepare_creds()); // need root to read flag
    int result = kernel_read(file, buf, count, &file->f_pos);
    revert_creds(old); // restore previous creds
    return result;
...
void load_header(vmstate_t *state, unsigned int new_cs) {
    ...
    state->regs.i = header->entry_point; // get from header
...
void sys_exec(vmstate_t *state, word_t addr)
{
    // addr is mem addr containing the value of the code id to swap to
    // e.g. if mem[0] = 0x41 and exec(state, 0) is executed then this will swap to cs = 0x41

    // this is just to provide something that meets the constraints of spectre v1

    // 1. has a bounds check protecting a secret (memlimit)
    // 2. peforms a memory access * <large constant> based on the dereferenced secret (header magic)
    // 3. that memory access is to a page userspace has access to

    // hopefully slightly more interesting than just throwing in an instruction that is just a v1 gadget

    unsigned int new_cs;

    // bounds check on mem limit that can be bypassed
    if (likely((float)addr / (float)state->memlimit < 1)) { // only allow access below memlimit
        new_cs = state->memory[addr + (state->regs.ds * PAGE_SIZE)];
        int header_magic = ((uint32_t*)(((char*)state->code) + (new_cs * PAGE_SIZE)))[0];
...
word_t read_memory(vmstate_t *state, word_t address)
{
    ...
    if (likely((float)address / (float)state->memlimit < 1)) {
        return memory[address];
    }
    return 0;
}

// shouldn't really matter if they can write to the restricted memory?
// could be useful, write to flag mem to make sure it stays in cache
...
void interpret_sys(vmstate_t *state, INSTRUCTION_ARGS)
{
    ...
    {
        TRACE("[s] ... open\n");
        if (state->regs.a >= state->memlimit) {
            write_register(state, INSTRUCTION_ARG2, -1);
        } else {
            write_register(state, INSTRUCTION_ARG2, sys_open(state, (char*)(state->memory + (state->regs.ds * PAGE_SIZE) + state->regs.a), state->regs.b, state->regs.c));
        }
    }
    ...
    {
        TRACE("[s] ... read_memory\n");
        if (state->regs.b < state->memlimit) { // only allow read into restricted memory
            write_register(state, INSTRUCTION_ARG2, -1);
        }
        else {
            char *read_buffer = (char *)(state->memory + (state->regs.b + (state->regs.ds*PAGE_SIZE)));
    ...
    // {
    //     TRACE("[s] ... write\n");
    //     if (state->regs.b >= state->memlimit) { // only allow write from un-restricted memory
    //         write_register(state, INSTRUCTION_ARG2, -1);
    //     }
    //     else {
    //         char *write_buffer = (char *)(state->memory + (state->regs.b + (state->regs.ds*PAGE_SIZE)));
    //         word_t write_length = MIN(state->regs.c, state->memlimit-state->regs.b); // min (count, memsize-addr)
...
static int device_open(struct inode *inode, struct file *file)
{
    ...
    context->mem = kvzalloc(MEM_PAGES*PAGE_SIZE, GFP_KERNEL);
