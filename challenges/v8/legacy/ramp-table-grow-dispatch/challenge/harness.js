...
table.set(0, functions.safe);
const validator = { copiedIndex: -1 };
function copyLiveIndex(index) { validator.copiedIndex = index | 0; }
function validate() {
    const fn = table.get(validator.copiedIndex);
    return validator.copiedIndex > 0 && fn && fn() === token ? token : 0;
}
const api = Object.freeze({ table, safeFunction: functions.safe, winFunction: functions.win, copyLiveIndex, validate });

__PWN_SOLVE_SOURCE__;

if (typeof solve !== "function") throw new Error("define solve(api)");
solve(api);
if (validate() === token) print("__PWN_SUCCESS_MARKER__");
