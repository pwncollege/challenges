...
That repetition is a _loop_, which you practiced in [Writing Loops](/computing-101/control-flow/loop-write) and will adapt here.
...
So, your loop is: look at the next byte, if it's `0`, jump beyond the loop (look back at [Writing Loops](/computing-101/control-flow/loop-write) for reference), otherwise convert the digit just like `atoi_digit` did, multiply the total by `10`, add the digit, and loop back to the head of the loop.
