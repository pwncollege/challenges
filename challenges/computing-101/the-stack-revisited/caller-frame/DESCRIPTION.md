...
The secret you have to return isn't anywhere obvious. It's sitting on the **stack**, inside the caller's own frame. You have to reach over and grab it.

## How is that possible?

Because of what `call solve` actually did. Let's walk through it.

Right before the grader's `caller` function executed `call solve`, its stack looked like this:
...
```text
   ...
   ...
   ... the secret lives somewhere in here ...
...
1. Push the return address onto the stack (8 bytes). **Pushing decrements `rsp`**, so the return address ends up at a *lower* address than what was already on the stack.
2. Jump to your code.

That first step is what "the stack grows downward" actually means.
Every push, every `call`, every `sub rsp, N` — they all move `rsp` *down* in memory.
Whatever was on the stack before you arrived sits at *higher* addresses than your current `rsp`.
...
The caller's locals are *above* your `rsp`. The secret is *above* your `rsp`. To find it, you index into memory with a **positive** offset from `rsp`.

If you go the other way — negative offsets, *below* `rsp` — you'll find unallocated stack space at best, and a guard-page segfault at worst. There's nothing useful for you down there. The lesson here is structural: **your caller is above you**, because the act of calling you pushed the stack downward.

## The task

The grader's `caller` is laid out very specifically (it's hand-written assembly so the offsets are guaranteed). When your `solve` starts running, the layout is:
