 ...
 * `enter_solve` seeds the callee-saved registers rbx, rbp, r12, r13, r14, r15 with
 ...
 *   - solve must then restore rbx,rbp,r12-r15 and rsp before returning. After
 *     solve returns, enter_solve snapshots them, safely restores its own stack,
 *     and we check the snapshot against the state we supplied.
 *
...
extern uint64_t expected[6];   /* sentinels placed in rbx,rbp,r12,r13,r14,r15 */
extern uint64_t seen[6];       /* those registers as solve left them */
extern uint64_t seen2[6];      /* values check_callee_clobbered saw */
extern uint64_t incoming[8];   /* unspecified caller-saved inputs */
extern uint64_t expected_rsp;  /* rsp immediately before calling solve */
extern uint64_t seen_rsp;      /* actual rsp immediately after solve returned */
...
static const char *regname[6] = {"rbx", "rbp", "r12", "r13", "r14", "r15"};

static int randomize_incoming(void) {
    if (getrandom(incoming, sizeof incoming, 0) != (ssize_t)sizeof incoming) {
    ...
    for (size_t i = 0; i < sizeof incoming / sizeof incoming[0]; i++)
        incoming[i] = (incoming[i] & 0x0000ffffffffffffULL) | 0x5a5a000000000000ULL;
    return 0;
}
...
void __attribute__((force_align_arg_pointer)) check_clobber_c(void) {
    for (size_t i = 0; i < sizeof seen2 / sizeof seen2[0]; i++) {
        if (seen2[i] != WORK_VALUE) {
            printf("when you called check_callee_clobbered, %s held 0x%lx, not 0x%x --- "
                   "put your work (0x%x) in all of rbx, rbp, r12, r13, r14, r15 first.\n",
...
int main(int argc, char **argv) {
    ...
    LOG("  rdi = check_callee_clobbered (confirms you put 0x%x in rbx, rbp, r12, r13, r14, r15)", WORK_VALUE);
    LOG("  rbx, rbp, r12, r13, r14, r15 arrive holding MY values --- give them back unchanged");

    enter_solve(solve, check_callee_clobbered);

    if (!clobber_ok) {
        LOG("solve never set rbx, rbp, r12-r15 to 0x%x and called check_callee_clobbered.", WORK_VALUE);
        return 1;
    }
    ...
    if (seen_rsp != expected_rsp) {
        LOG("solve returned with rsp = 0x%lx, but its caller expected 0x%lx.", seen_rsp, expected_rsp);
