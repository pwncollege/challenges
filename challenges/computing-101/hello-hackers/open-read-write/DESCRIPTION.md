...
3. `open` it (syscall `2`)
4. `read` enough bytes from the returned fd into memory to include the flag. The returned fd will be stored in `rax`; you'll need to move that to `rdi` for `read`'s first argument. Make sure to do this _before_ you set the syscall number for read!
