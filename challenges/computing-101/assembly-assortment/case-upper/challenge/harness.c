...
int main(int argc, char **argv) {
    ...
    LOG("resolving `str_upper` symbol ...");
    void (*str_upper)(char *) = (void (*)(char *))dlsym(h, "str_upper");
    if (!str_upper) {
        LOG("missing `str_upper` symbol --- did you `.global str_upper` in your assembly?");
        return 2;
    }
    LOG("calling str_upper(s) --- your code transforms the %zu-byte string in place", n);
    str_upper(buf);
    LOG("str_upper returned; reporting the modified bytes to the checker");
